...
import type { AcpAdapter } from "@/infra/acp/acp.adapter";
import { NoteMentionService } from "@/infra/obsidian/mention-service";
...
import { AcpContextProvider } from "@/features/chat/context/AcpContext";
import { EditorController } from "@/app/editor/editor-controller";
import { useAgentClient } from "@/features/chat/hooks/useAgentClient";
import {
	hasMentions,
	cleanMentionSyntax,
} from "@/infra/obsidian/mention-utils";
...
import { SuggestionItem } from "@/features/chat/components/SuggestionPopover";
import { ChatInput } from "@/features/chat/components/ChatInput/ChatInput";
import PermissionDialog from "@/features/chat/components/PermissionDialog";
...
export const ChatPanel: React.FC<ChatPanelProps> = ({ app, plugin }) => {
	...
	const {
		...
		app,
		{ enabled: chatContext.mode === "agent" },
	);
	...
	const handleNewAPIChat = async () => {
		setShowNewChatMenu(false);
		const availableModels = getAvailableModels();
		const selectedApiModel =
			availableModels.find((model) => model.id === selectedModel) ||
			availableModels[0] ||
			null;

		const modelId = selectedApiModel?.id || "";
		setSelectedModel(modelId);
		...
		if (selectedApiModel) {
			...
			const newProvider =
				providerMap[selectedApiModel.provider] || AIProviderType.BYOK_OPENAI;
			plugin.settings.provider = newProvider;
			plugin.settings.activeChatModelId = selectedApiModel.id;

			if (selectedApiModel.provider === "openai" && selectedApiModel.name) {
				plugin.settings.openaiModel = selectedApiModel.name;
			} else if (
				selectedApiModel.provider === "gemini" &&
				selectedApiModel.name
			) {
				plugin.settings.geminiModel = selectedApiModel.name;
			} else if (
				selectedApiModel.provider === "deepseek" &&
				selectedApiModel.name
			) {
				plugin.settings.deepseekModel = selectedApiModel.name;
			}
	...
	const handleNewAgentChat = async (agentConfig: ChatModelConfig) => {
		...
		// Cancel any existing agent session before starting a new one
		if (agentClient && agentSessionId) {
			try {
				await agentClient.cancel(agentSessionId);
			} catch (e) {
				console.warn("[ChatPanel] Failed to cancel agent session:", e);
			}
		}
