...
import type { SessionModelState } from "@/core/models/session-update";
import type {
	IAgentClient,
	Subscription,
} from "@/core/ports/agent-client.port";
import type { MyPluginSettings } from "@/app/settings/plugin-settings";
import { AIProviderType } from "@/app/settings/plugin-settings";
import {
	getActiveAgentConfig,
	getAvailableAgentsFromChatModels,
	toAcpAgentConfig,
} from "@/agent/acp-agent-config";
		...
		async function init() {
			if (settings.provider === AIProviderType.ACP_LOCAL) {
				...
				try {
					...
					await adapter.initialize(toAcpAgentConfig(agentConfig));
