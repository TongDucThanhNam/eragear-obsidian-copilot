...
class DiffWidget extends WidgetType {
	...
	toDOM() {
		const wrap = createEl("span", { cls: "eragear-diff-widget" });

		// Accept Button
		const accept = createEl("button", {
			cls: "eragear-diff-btn eragear-btn-accept",
			text: "✓",
			attr: {
		...
		const reject = createEl("button", {
			cls: "eragear-diff-btn eragear-btn-reject",
			text: "✗",
			attr: {
...
export const diffViewExtension = [diffStateField, diffPlugin];
