...
import type { AgentConfig } from "@/core/models/agent-config";
...
import {
	...
	type Subscription,
} from "@/core/ports/agent-client.port";
...
export class AcpAdapter implements IAgentClient {
	...
	async newSession(workingDirectory: string): Promise<NewSessionResult> {
		...
		try {
			...
			console.log("[AcpAdapter] New session created:", result);
			...
			if (result.models) {
				console.log("[AcpAdapter] Session models raw:", result.models);
				models = {
	 ...
	 *
	 * NOTE: Authentication is not used for this plugin.
	 * Always returns true.
	 */
	async authenticate(methodId: string): Promise<boolean> {
		console.log("[AcpAdapter] authenticate() not used, methodId:", methodId);
		return true;
	}
	...
	/**
	 * Load a saved session and restore it.
	 * Note: This creates a NEW session with loaded content, not using agent's loadSession.
	 *
	 * DEPRECATED: Use loadSessionFromVault() instead for full loading.
	 * This method remains for backward compatibility.
	 */
	async loadSession(sessionId: string): Promise<NewSessionResult> {
		...
		// Create a new session with same working directory
		const connection = this.ensureReady();
		const result = await connection.newSession({
		...
		// Replay messages by emitting session updates
		const updates = this.sessionManager.messagesToUpdates(sessionData.messages);

		for (const update of updates) {
			this.notifySessionUpdate(update);
		}

		console.log("[AcpAdapter] Loaded session content:", sessionId);

		// Return session info with metadata
		const modes = result.availableModes || [];
		return {
			sessionId: result.sessionId,
			availableModes: modes.map((m) => ({
				id: m.id,
	...
	async listSessions(): Promise<SessionSummary[]> {
		...
		try {
			...
			console.error("[AcpAdapter] Failed to list sessions:", e);
			return [];
		}
	}

	/**
	 * Save current session to vault as markdown.
	 */
	async saveCurrentSession(
		...
		const activeSession = {
			...
			updatedAt: new Date(),
			title,
			workingDirectory: this.currentConfig?.workingDirectory || "",
		...
		await this.sessionManager.saveSession(activeSession);

		console.log("[AcpAdapter] Saved session:", sessionId);
